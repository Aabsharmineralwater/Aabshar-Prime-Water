...
import { Home, Package, Building2, Users, Phone } from 'lucide-react';
...
export default function BottomNav({ onLinkClick }: BottomNavProps) {
  ...
  return (
    <div className="fixed bottom-3 left-2 right-2 sm:left-4 sm:right-4 z-40 bg-slate-950/98 backdrop-blur-3xl border border-sky-400/25 shadow-[0_15px_45px_rgba(2,132,199,0.35),0_4px_12px_rgba(0,0,0,0.5)] rounded-2xl md:hidden pb-safe overflow-hidden">
      <div className="flex items-center justify-around h-16 px-1 max-w-lg mx-auto">
        ...
        >
          <div className={`p-1.5 rounded-xl transition-all duration-300 ${
            activeSection === '#hero' 
              ? 'text-sky-200 bg-sky-500/25 scale-105 shadow-[0_0_15px_rgba(14,165,233,0.3),inset_0_1px_4px_rgba(255,255,255,0.2)] border border-sky-400/30' 
              : 'text-slate-400 hover:text-slate-200'
          }`}>
            <Home className="w-5 h-5 stroke-[2.2]" />
          </div>
          <span className={`text-[9px] tracking-wide font-black transition-colors duration-300 ${
            activeSection === '#hero' ? 'text-white font-extrabold' : 'text-slate-450 font-bold'
          }`}>Home</span>
          {activeSection === '#hero' && (
            <span className="absolute bottom-0 w-6 h-1 rounded-full bg-linear-to-r from-brand-teal to-brand-aqua shadow-[0_0_14px_#38bdf8] animate-fade-in" />
          )}
        </button>
        ...
        <button
          ...
          className="flex flex-col items-center justify-center flex-1 h-full gap-0.5 transition-all duration-300 relative select-none active-scale py-1 cursor-pointer border-0 group"
          style={{ minWidth: '40px', minHeight: '44px' }}
        >
          <div className="p-1.5 rounded-xl transition-all duration-300 text-slate-400 group-hover:text-slate-200">
          ...
          <span className="text-[9px] tracking-wide font-bold transition-colors duration-300 text-slate-450 group-hover:text-slate-200">
            WhatsApp
          </span>
        </button>
